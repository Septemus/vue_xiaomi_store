    ...
    <div>
      <b-navbar toggleable="lg" type="dark" variant="dark">
        <b-navbar-brand href="javascript:void(0);" @click="$router.push({
          name:'front_page'
        })"><img src="../assets/images/logo-mi2.png" alt="" class="w-25  d-block"></b-navbar-brand>
          ...
          <b-navbar-nav>
            ...
            <template v-if="!this.$store.state.username">
              <b-nav-item href="javascript:void(0);"
                @click="$root.$emit('bv::show::modal', 'myModal'); $store.commit('modal', 1)">登录</b-nav-item>
              <b-nav-item href="javascript:void(0);"
                @click="$root.$emit('bv::show::modal', 'myModal'); $store.commit('modal', 2)">注册</b-nav-item>
            </template>
            <template v-else>
              <b-nav-item href="javascript:void(0);">
